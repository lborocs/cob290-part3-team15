...
function LeaveModal ({open, onClose, leaveFunction, closeDropdown, refs, floatingStyles, isSelf, chatName}) {
    ...
    return (
        <Modal open={open} onClose={handleClose} refs={refs?.setFloating} floatingStyles={floatingStyles}>
            ...
            <p className="text-base text-gray-700 mb-6 leading-relaxed">{!isSelf? (`Messages will still be stored as they are and you will be able to recover the chat by re-adding the user.`):(<>You are about to delete the group <strong>{chatName}</strong>, this is an irreversible process.<br/>Are you sure you want to continue?</>)}</p>
            <hr className="border-t border-gray-300 mb-6" />
